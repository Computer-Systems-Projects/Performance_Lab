...
cs1300bmp_readfile(char *filename, struct cs1300bmp *image)
{
  ...
  if ( error ) {
	...
	image -> red[row][col] = (int) rarray[row * width + col];
	image -> green[row][col] = (int) garray[row * width + col];
	image -> blue[row][col] = (int) barray[row * width + col];
      }
    }
    //
    //  Free the memory.
...
cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  for (int row = 0; row < height; row ++ ) {
    for (int col = 0; col < width; col ++ ) {
      rarray[row * width + col] = (unsigned char) image -> red[row][col];
      garray[row * width + col] = (unsigned char) image -> green[row][col];
      barray[row * width + col] = (unsigned char) image -> blue[row][col];
    }
  }
